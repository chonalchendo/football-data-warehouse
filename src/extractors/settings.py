...
class TransfermarktConfig(BaseSettings):
    ...
    ITEM_PIPELINES: dict[str, int] = Field(default_factory=lambda: {"transfermarkt.transfermarkt.pipelines.TransfermarktGCSPipeline": 300})
    FEEDS: dict[str, Any] = Field(
        default_factory=lambda: {"gs://football-data-warehouse/raw/transfermarkt/{season}/{name}.parquet": {"format": "parquet"}}
    )
...
class ScraperConfig(BaseSettings):
    transfermarkt_extract: TransfermarktConfig = Field(default_factory=TransfermarktConfig)
